	...
	<div class="uk-card uk-card-hover uk-card-default uk-card-small uk-animation-fade l1card">
		<div class="uk-card-header uk-grid-small" uk-grid>
			<div class="uk-width-expand@m">
				<h3 class="uk-card-title sikd-blue-text uk-float-left uk-margin-remove uk-text-uppercase"><span id="title"></span> Per Provinsi Tahun <span id="tahun">{!! $yearParam !!}</span></h3>
			</div>
				...
				<div uk-dropdown class="uk-height-large uk-max-height-large uk-overflow-auto">
		            <ul class="uk-nav uk-nav-default uk-dropdown-nav">
		                ...
		                @foreach($postures as $posture)
		                    <li id="tkdd{!! $posture->idPostur !!}"><a href="{!! url('level-1/tkdd/' . $posture->idPostur . '/' . $yearParam) !!}">{!! $posture->uraianPosturSingkat !!}</a></li>
		                @endforeach
